...
test_task2_entry(void *arg)
{
    ...
    logger_info("Test Task 2 started on CPU %u (preemptive scheduling)\n", cpu_id);
...
t_main_entry()
{
    ...
    if (cpu_id == 0) {
        ...
        if (!test1) {
            logger_error("Failed to create test task\n");
        } else {
            logger_info("Created preemptive test task on CPU 0\n");
        }
    }
