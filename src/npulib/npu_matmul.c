                ...
                npu_dpu_desc  *dpu_desc)
{
    ...
    value   = ((cna_desc->data_sign & 0x1) << 3) | ((cna_desc->cvt_type & 0x1) << 1) |
...
gen_matmul_fp16(matmul_params_t *params)
{
    ...
    fd_bytes =
        cna_desc.datain_width * cna_desc.datain_height * cna_desc.datain_channel * sizeof(__fp16);
    ...
    weight_banks =
        ((cna_desc.weight_bytes % NPU_CBUF_BANK_SIZE) == 0) ? weight_banks : weight_banks + 1;
    ...
    dpu_desc.surf_add =
        (!params->fp32tofp16) ? dpu_desc.dst_surf_stride * 4 : dpu_desc.dst_surf_stride * 2;
...
gen_matmul_int8(matmul_params_t *params)
{
    ...
    fd_bytes =
        cna_desc.datain_width * cna_desc.datain_height * cna_desc.datain_channel * sizeof(int8_t);
    fd_banks     = (fd_bytes / NPU_CBUF_BANK_SIZE);
    fd_banks     = ((fd_bytes % NPU_CBUF_BANK_SIZE) == 0) ? fd_banks : fd_banks + 1;
    weight_banks = (cna_desc.weight_bytes / NPU_CBUF_BANK_SIZE);
    weight_banks =
