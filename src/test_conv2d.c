/*
 * test_conv2d.c — Conv2D NPU hardware test for RK3588
 *
 * Tests a small INT8 conv2d on the NPU and compares against CPU reference.
 * Uses NC1HWC2 data layout and tiled weight layout.
 */
                ...
                for (int kh = 0; kh < KH; kh++) {
                    for (int kw = 0; kw < KW; kw++) {
                        ...
                        for (int ic = 0; ic < IN_C; ic++) {
                            int8_t input_val  = input_data[ih * IN_W * IN_C + iw * IN_C + ic];
                            int8_t weight_val = weight_data[oc * KH * KW * IN_C + kh * KW * IN_C + kw * IN_C + ic];
...
void
rknpu_test_conv2d(void)
{
    logger_info("=== Conv2D NPU Test ===\n");
    ...
    if (!regcmd || !tasks || !input || !weights || !output) {
        logger_error("Conv2D Test: Memory allocation failed\n");
        return;
    }
    ...
    if (gen_conv2d_int8(&params) != 0) {
        logger_error("Conv2D Test: gen_conv2d_int8 failed\n");
        return;
    }
    ...
    for (int i = 0; i < OUT_C * KH * KW * IN_C; i++)
    ...
    /* Layout weights using tiled format */
    int8_t *wt = (int8_t *)weights;
    for (int oc = 0; oc < OUT_C; oc++) {
        for (int kh = 0; kh < KH; kh++) {
            for (int kw = 0; kw < KW; kw++) {
                for (int ic = 0; ic < IN_C; ic++) {
                    int src_idx = oc * KH * KW * IN_C + kh * KW * IN_C + kw * IN_C + ic;
                    int dst_idx = conv2d_weight(IN_C, KH, KW, OUT_C, oc+1, ic+1, kh, kw, 1);
    ...
    logger_info("Submitting conv2d task to NPU...\n");
    rknpu_submit_task(&submit);

    invalidate_dcache_va_range(regcmd, 1024 * 1024 * 16);
    logger_info("NPU conv2d completed.\n");
    ...
    if (mismatches == 0) {
        logger_info("Conv2D %dx%dx%d k%dx%d → %dx%dx%d: ALL PASSED\n",
                    IN_H, IN_W, IN_C, KH, KW, OUT_H, OUT_W, OUT_C);
    } else {
        logger_error("Conv2D: %d mismatches out of %d\n",
